 ...
 * @author Kansio
 */
@ModuleInfo(name = "Damage Particles", description = "Shows particles when you take damage", category = Category.RENDER, visibility = false)
public class DamageParticles extends Module {
    ...
    @EventLink
    Listener<Render2DEvent> render2DEventListener = event -> {
        try {
            for (hit h : hits) {
                if (h.isFinished()) {
                    hits.remove(h);
